 ...
 */
export const TrpExportTemplate: React.FC<TrpExportTemplateProps> = ({ viewModel }) => {
  ...
  const formatObservacoes = (obs: string | null): string[] => {
    ...
    // Quebrar por ponto, vírgula ou quebra de linha
    return obs
      .split(/[.;]\s*|\n/)
      .map(line => line.trim())
      ...
      <article style={documentStyles}>
        ...
        {sectionsWithFields.map(({ section, fields }, sectionIndex) => {
          ...
          if (section.title === 'OBSERVAÇÕES' && fields.length > 0) {
            const observacoesField = fields.find(f => f.fieldName === 'observacoes');
            const observacoesValue = observacoesField?.value as string | null | undefined;
          ...
          if (section.title === 'ASSINATURAS' && fields.length > 0) {
            return (
              <section key={section.title}>
                ...
                {(() => {
                  const dataField = fields.find(f => f.fieldName === 'data_assinatura');
                  return (
          ...
          // Seção especial: Atesto (sempre após Observações)
          if (section.title === 'CONDIÇÕES DE RECEBIMENTO' && sectionIndex === sectionsWithFields.findIndex(s => s.section.title === 'CONDIÇÕES DE RECEBIMENTO')) {
            const observacoesIndex = sectionsWithFields.findIndex(s => s.section.title === 'OBSERVAÇÕES');
            const atestoIndex = observacoesIndex >= 0 ? observacoesIndex + 1 : sectionIndex + 1;
            ...
            return (
              <React.Fragment key={section.title}>
                  ...
                  <table style={tableStyles}>
                    ...
                    <tbody>
                      {fields.map((field, fieldIndex) => {
                        ...
                        const rowStyle = isEven ? tableRowEvenStyles : tableRowStyles;
                        const isObjeto = field.fieldName === 'objeto_contrato';
                        const cellValueStyle = isObjeto ? objetoStyles : tableCellValueStyles;
                        
                        return (
                          <tr key={field.fieldName} style={rowStyle}>
                            <td style={tableCellLabelStyles}>
                              {field.label}
                            </td>
                ...
                <section>
                  <h2 style={h2Styles}>
                    {atestoIndex + 1}. ATESTO
                  </h2>
                  <blockquote style={atestoStyles}>
                    Atesto que o recebimento foi realizado conforme as condições descritas neste documento,
                    estando os bens/serviços em conformidade com o objeto contratual e as especificações
